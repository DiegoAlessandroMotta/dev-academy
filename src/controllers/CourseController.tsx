...
class CourseController {
  ...
  getCourseModalContent(courseId: string): {
    ...
    if (courseId === '1' && course.levels) {
      return {
        ...
        content: this.generateCppCourseContent(course),
      }
    } else if (courseId === '2') {
      return {
        title: 'INFORMACION DEL CURSO DE DISEÑO DE PAGINAS WEB',
        whatsappLink: course.whatsappLink,
        content: this.generateWebDesignCourseContent(),
      }
    }
  ...
  generateCppCourseContent(course: Course): JSX.Element {
    return (
      <div>
        <h1>{course.title}</h1>
        <p>
        ...
        <p>
          ...
          un sistema que puede ser otorgado por el docente o eleccion de si mismo(considerar todo lo avanzado).&quot;
        </p>
        <br />
        ...
        <br />
        <p>LOS CERTIFICADOS SERAN OTORGADOS A LOS ALUMNOS QUE MAS HALLAN DESTACADO .</p>
      </div>
    )
  }

  private generateWebDesignCourseContent(): JSX.Element {
    return <div>Información del curso de diseño de páginas web</div>
  }
}
